...
```python
from mantis_model_iteration_tool import fetch_assets, DataProvider, evaluate
...
```python
from mantis_model_iteration_tool import BREAKOUT_ASSETS
...
Data is cached to `mantis_model_iteration_tool/.data/` on first fetch (configurable via `MANTIS_DATA_DIR`).
